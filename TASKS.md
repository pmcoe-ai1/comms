...
Last updated: 2026-03-06T22:45:00Z
---
...
| TASK-64 | Fix check-stock-on-add-item.filled.yaml — action references non-existent operation validate-stock-availability | ⚠ IN PROGRESS | -- | -- | Pre-existing issue surfaced by TASK-55 CI gate enforcement. fill.action references operation 'validate-stock-availability' which does not exist in example.canonical-model.yaml. Gate Pass 1 rejects it. 3 consecutive CI failures on main. Fix: change action to call-operation: add-order-item (exists in canonical model, matches existing src impl comment). |
