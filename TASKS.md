...
Last updated: 2026-03-06T21:30:00Z
---
...
| ID | Task | Status | Depends on | Blocked on | Notes |
|---|---|---|---|---|---|
| TASK-53 | Add expectedResult to all 11 scenarios in subscription-billing.canonical-model.yaml -- required for gate Pass 2 to work on subscription filled templates | ✗ NOT DONE | -- | -- | VERIFY-06 root cause. Example model has 14 expectedResult fields; subscription model has 0. Gate Pass 2 fails with "expected undefined" on all subscription templates. Must fix before gate Pass 2 can be claimed operational for subscription domain. |
| TASK-54 | Add operation stubs to codegen output (generated/operations/ directory) per FABRIC section 8 "Typed function signatures for every operation" | ✗ NOT DONE | -- | -- | MISSING artifact. generated/operations/ not present. No prior TASK. Note: full operation stubs importing dependency adapters depend on TASK-29 (Phase 2). Basic stubs with types from generated/interfaces/ can be done now. |
| TASK-55 | Fix CI gate job to pass filled templates as positional args so Pass 1-4 all execute -- currently only Pass 3-4 run in CI | ✗ NOT DONE | -- | -- | VERIFY-07. CI job labeled "Enforcement Gate (Pass 1-4)" but node gate.js --model <file> triggers Pass 3-4 only. Add steps: node gate.js templates/check-stock-on-add-item.filled.yaml --model files/example.canonical-model.yaml and equivalent for subscription templates. |
| TASK-56 | Add template-generator stage to CI pipeline so templates are regenerated when canonical model changes | ✗ NOT DONE | -- | -- | FABRIC section 6.1 lists template-generator as pipeline stage. CI has no template-gen job. Templates are pre-committed. Canonical model changes do not trigger template regeneration in CI. Add job: node files/template-generator.js <model> --output-dir templates/ after validate step. |
