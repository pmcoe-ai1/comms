...
| TASK-66 | gate.js initialState added unconditionally — allows invalid transitions when source state is known | ✗ NOT DONE | -- | -- | TASK-65 fix at gate.js:327-328 adds initialState to validNextStates unconditionally. When source states are known (BUG-023 path at line 316-322), initialState should NOT be added. Fix: only add initialState when sourceStates.length === 0. |
